...
| test_ksh93       | **partially fixed, further blockers found and fixed** — mamprobe's C-compiler sniff, `ast_wchar.h`'s `#include_next <wchar.h>` reaching glibc, and ksh93's own `std/stdio.h`→`ast_stdio.h` (`__FILE`) forwarding were fixed in the sessions below; two more root-caused this session: `resolve_include()`'s own RCC_INCDIR self-reference collision (blocked `comp/iconv.c`'s `iconv_t` and `string/chresc.c`'s `CC_bel`/`CC_esc`/`CC_vt`) and a genuine rcc SIGSEGV in `add_type_internal()`'s `ND_COMMA` case (crashed on `sfio/sfvprintf.c`) — see "Fixed (2026-08-10, include_next self-reference / ND_COMMA null-deref session)" below. Build now compiles and links the **entire** `libast` library and reaches `cmd/INIT`'s own `iffe` self-test suite (159/161 passing, 2 narrow iffe-internal struct-introspection failures — not yet investigated, a much deeper stage than any prior session reached) |

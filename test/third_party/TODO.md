...
- **`<stdbit.h>` exposed the leading/trailing-zero and popcount C23 APIs
  but not `stdc_bit_width` or its suffixed forms** -- `include/stdbit.h`.
  GNU Emacs 30.2's `src/lisp.h` calls `stdc_bit_width (unsigned long long)`
  in `elogb()`, so configure accepted the bundled header but compilation
...
- **`cast_funcall_args()` checked argument types but never the argument
  COUNT against a prototyped function's parameter list** -- `src/parser.c`.
  A wrong-arity call like `pthread_setname_np ("a")` against glibc's
  two-parameter prototype compiled silently, so GNU Emacs's configure
  probe "does pthread_setname_np take a single argument" (which relies on
  the compile error) misdetected `HAVE_PTHREAD_SETNAME_NP_1ARG` on glibc.
  Every `make-thread` then emitted the one-argument call
  `pthread_setname_np (p_name)`, and glibc read the missing `pthread_t`
  from a garbage register -- SIGSEGV in `strlen` on the bogus name
  pointer, crashing emacs's `src/thread-tests`, `src/process-tests`
  (`process-tests/multiple-threads-waiting`), `src/emacs-module-tests`
  and `lisp/thread-tests`. Fixed by counting declared parameters vs. call
  arguments in `cast_funcall_args()` and erroring on a mismatch (C11
  6.5.2.2p6/p7), while keeping variadic and old-style-K&R calls lenient.
  Regression cases in `test/test_funcall_argcount.c`. With `config.h`
  regenerated by the fixed rcc, `HAVE_PTHREAD_SETNAME_NP_1ARG` is now
  correctly undefined and all four emacs test files pass their full
  suites; `test_emacs` still has unrelated network-dependent failures
  (tramp/erc/eglot/gnutls) in `make check`.
...
= v, .refs = {a, b, c}}` — a nested struct member that is itself an
  ...
  range-designator variant, an explicit nested `[idx]` designator and
  wide-element sign-extension), PASS on x86-64, ARM64 and mingw.
...
- **`eval_const_expr()`'s `ND_LVAR` case read a struct/union/array-typed
  ...
  file-scope-static cases), `test/test_double_const_shift.c`,
  ...
  `test/test_math_constants_gnu.c` (all new). PASS at
  -O0..-O3 on x86-64, ARM64 (qemu-aarch64) and mingw (wine); `make

...
- **test_got now passes** — got-portable builds with rcc and all 7
  decomposed sub-tests (compat, regress-delta, regress-deltify,
  regress-fetch, regress-idset, regress-path, regress-tog) run green.
  The blocker was purely environmental: got's `configure` hard-requires
  `libtls` (LibreSSL's TLS library) via pkg-config, and Debian/Ubuntu
  dropped LibreSSL from their repos. `test/linux_thirdparty.bash` now
  has a `shared_libressl()` helper (mirroring `shared_muon`/
  `shared_binutils`) that builds LibreSSL 4.3.2 into a local
  `libressl-install` prefix and exports `PKG_CONFIG_PATH`, and
  `test_got()` calls it before `./configure`. The earlier TODO row
...
- \*\*test*gnutls now passes completely (rc=0). Three rcc bugs + one
  harness bug were fixed this session on top of the two earlier ones
  (duplicate DT_NEEDED, block-scope conflicting redeclaration):
  (1) `declaration()` reset the pending constructor/destructor flags at
  every statement-level declaration, so the FIRST local declaration
  inside `static void __attribute__((constructor)) f(void) { int x;
... }` wiped the pending constructor before the definition consumed
  it -- the constructor never landed in .init_array and never ran.
  gnutls' lib_init() (which builds the embedded ASN.1 trees via
  ...
  that (all the resume-*, system-override-\_, pkcs11/\*,
  mini-global-load, tls-supplemental, ... failures, 35 tests) failed.
  Fixed by capturing the flags before parsing the body. Regression:
  `test/test_ctor_body_decl.c`. (2) -shared internal calls to defined
  global/weak functions bound directly instead of through the PLT, and
  non-shared executables didn't export symbols the linked libraries
  reference -- so gnutls' GNUTLS_SKIP_GLOBAL_INIT (the test exe's
  strong \_gnutls_global_init_skip overriding the lib's weak one) failed
  (tests/global-init-override: "Library is already initialized").
  Fixed in link_elf.c (interposable symbols get PLT entries + the exe
  exports .so-referenced definitions) and codegen.c (same-TU
  GLOBAL/WEAK calls emit a PLT32 reloc; only STATIC calls are patched
  directly). Regression: `test/test-link.sh` case 18. (3)
  `run_batch.sh` exported TIMEOUT (the per-test timeout) into the test
  environment, colliding with gnutls-cli-debug.sh's own `$TIMEOUT`
  timeout command (`: ${TIMEOUT=timeout}`) -- "1800: command not
  found", identical on a gcc build; fixed by unsetting TIMEOUT inside
  the test subshell. The full gnutls build + `tests` suite + `fuzz`
  now pass.
...
c-header` spellings, confirms a `.gch` file is produced, and confirms
  ...
  `test_git` (2026-08-27, t1013-read-tree-submodule 58/58 + `git
  status` segfault), `test_glib` (2026-08-26, resources/constructor-
  destructor fixes; remaining `file` test_measure mismatch is a
  gvfs-daemon environment artifact), `test_utillinux` (2026-08-25,
  builds clean + 134/134 blkid sub-tests) and `test_emacs` (build +
  test-secure-hash + full build OK; the `bootstrap-emacs.pdmp` runtime
  step only lacks some environment packages in this sandbox, not an
  rcc bug) are all fixed (2026-08-27).
...
| test_got         | **fixed** (2026-08-27) — was: configure "Couldn't find libtls" (LibreSSL's TLS lib, dropped from Debian/Ubuntu); the recipe now builds LibreSSL into a shared prefix (shared_libressl) and got builds + all 7 sub-tests (compat, regress-delta/deltify/fetch/idset/path/tog) pass with rcc                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
| test_gnutls      | **PASS** (2026-08-27) — full build + `tests` suite + `fuzz` green (rc=0). Was: duplicate DT*NEEDED from repeated `-lm` (config.h soname corruption, fixed in link_elf.c); block-scope conflicting function redeclaration not diagnosed (configure's ioctl probe wrongly passed, generated sys/ioctl.h took the SYS branch, fixed in parser.c func_decls_conflict); constructor functions with a local declaration in the body lost their .init_array entry (35 test failures: resume-*, system-override-\_, pkcs11/\*, mini-global-load, tls-supplemental, ...; fixed in parser.c); weak-symbol interposition broken in -shared links (global-init-override; fixed in link_elf.c + codegen.c); and the batch harness leaked its TIMEOUT env var into gnutls-cli-debug.sh (gcc-identical, fixed in run_batch.sh). See the three "Fixed (2026-08-27, test_gnutls ...)" sections above. Regression tests: test/test_ctor_body_decl.c, test/test_err_func_conflict_local.c, test/test_static_str_int_cast.c, test-link.sh cases 17+18                                                                                             |

...
| test_perl         | **codegen bug fixed, batch run pending** — the spill-slot/locals-offset collision root-caused in "Investigated, not fixed (2026-08-10, register-spill/locals-collision session)" below is now fixed (see "Fixed (2026-08-10, continued — spill/locals collision)" below); `test/test_spill_locals_collision.c` reproduces and verifies it, `make check-all` is clean, but the actual Perl build/`make test` wasn't re-run this session — next third-party batch run should confirm |
| test_go           | —                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
| test_nginx        | —                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
...
passing standalone on the mingw and arm64 cross targets. The real
`test_perl` third-party build (Perl's own `Configure`/`make test`) was
not re-run this session — too slow for a single-session turnaround —
so its TODO row below is left as a follow-up for the next scheduled
third-party batch run rather than claimed fixed on unverified
evidence.

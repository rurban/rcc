...
| test_ksh93       | **partially fixed, further blocker found** — mamprobe's C-compiler sniff and `ast_wchar.h`'s `#include_next <wchar.h>` reaching glibc were fixed in "Fixed (2026-08-10, output-extension-naming / include*next session)"; the `__FILE`-undeclared gap that blocked it next (glibc `<wchar.h>` needs `<stdio.h>`'s `__FILE` typedef, but rcc's own `include/stdio.h`'s `#include_next` was skipping ksh93's own `-Istd/stdio.h`→`ast_stdio.h`, the header that actually provides it) is now fixed too, see "Fixed (2026-08-10, include_next noop-forward refinement)" below — build gets much further (past `misc/fastfind.c`, `hash/*.c`, `string/ccmap*.c`, `port/getcodeset.c`). Now blocked by a \_different*, not-yet-investigated gap: `string/chresc.c` uses `CC_bel`/`CC_esc`/`CC_vt` (from the generated `ast_ccode.h`, via `include/ccode.h`) without including `ccode.h` itself — works on real gcc for a reason not yet root-caused, undeclared on rcc |

...
| test_gnutls      | **config.h soname corruption fixed** (2026-08-27) — was: duplicate DT_NEEDED entries from repeated `-lm` (rcc's default libm plus configure's LIBS) made the M_LIBRARY_SONAME probe capture `libm.so.6\nlibm.so.6`, writing a broken two-line define into config.h (991 "missing terminating quote" warnings per build); fixed in link_elf.c, see "Fixed (2026-08-27, test_gnutls -- duplicate DT_NEEDED from repeated -l flags)" above. Build now stops only at the upstream `src/gl/tests` vma-iter.o `conflicting types for 'ioctl'` (gnutls 3.8.13 bundled gnulib, `# if @SYS_IOCTL_H_HAVE_WINSOCK2_H@                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |     | 1`redeclares ioctl with`int request`vs glibc's`unsigned long`) — reproduces identically with gcc, not an rcc bug |

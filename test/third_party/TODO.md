...
| test_ksh93       | **partially fixed** — was: mamprobe's own C-compiler sniff test misreported rcc as "not a C compiler" (extension-naming bug) and `ast_wchar.h`'s `#include_next <wchar.h>` never reached the real system header (`wint_t` left undeclared) — both root-caused and fixed, see "Fixed (2026-08-10, output-extension-naming / include_next session)" below. Build now gets much further but still fails: the real `/usr/include/wchar.h` (now correctly reached) uses glibc-internal `__FILE`/`__file_unlocked`-style names rcc's parser doesn't recognize ("expected specific operator") — a separate, not-yet-investigated gap |
...
| test_perl         | **investigated, not fixed** — deep register-allocator dive triggered by a real segfault (`Perl_upg_version()` called with a NULL first arg from `S_enable_feature_bundle()`'s 9-level nested version-bundle ternary); root-caused a genuine spill-slot/locals-offset collision in `alloc_reg()`'s stack layout (`next_spill_slot` starts from a hardcoded 8, independent of `fn->stack_size`, so under enough concurrent register pressure a spilled register's address can land exactly on a live local's own offset), but the fix regressed 16 torture tests (variadic-function register-save-area and other stack-region collisions the same hardcoded-8 problem also touches) and was reverted rather than shipped half-verified; see "Investigated, not fixed (2026-08-10, register-spill/locals-collision session)" below for the full writeup and reproduction |
| test_go           | —                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
| test_nginx        | —                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |

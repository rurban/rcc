...
### Fixed (2026-08-26, python -- `#pragma pack` inside struct/union body)

- **rcc rejected `#pragma pack(...)` directives that appear inside a
  struct/union member-declaration list** (`src/parser.c`). Python's
  `_ctypes_test_generated.c.h` (generated by CPython's own test suite)
  uses the Microsoft-bitfield layout idiom:

  ```c
  #pragma pack(push, 1)
  struct __attribute__((ms_struct)) Example_gh_84039_good {
      #pragma pack(push, 1)
      struct __attribute__((ms_struct)) {
          uint8_t a0 :1; ... uint8_t a7 :1;
      } a;
      #pragma pack(pop)
      uint16_t b0 :4; uint16_t b1 :12;
  };
  #pragma pack(pop)
  ```

  Real GCC ignores such in-body pragmas (a struct's layout is fixed once
  parsing has begun, so an inner pack cannot retroactively change it).
  rcc's member-declaration loop had handling for `# pragma pack` at file
  scope but not inside a struct body, so it treated the leading `#` as
  the start of a member declaration, errored "expected specific
  operator", and aborted the whole Python build in `_ctypes_test.c`.
  Fixed by treating `# pragma pack(...)` and `# pragma fenv(...)` as
  no-ops inside the struct/union member loop. New regression test:
  `test/test_pragma_pack_in_struct_body.c`. Python now compiles much
  further; a later `_freeze_module` runtime crash is still under
  investigation.

- **rcc's `-E` mode concatenated adjacent string literals that were
  separated by source newlines** (`src/preprocess.c`). Phase 6 string
  concatenation belongs to the compilation proper, not to the
  preprocessor; real `cpp -E` preserves each literal on its own output
  line. Micropython's `py/makeqstrdata.py` parses `-E` output with the
  regex `^Q\((.*)\)$`, expecting one `Q("...")` item per line. rcc was
  joining all consecutive string literals (e.g. `"Q(*)"`, `"Q(_)"`,
  `"Q(/)"` on three source lines became `"Q(*)Q(_)Q(/)"` on one line),
  so the regex greedily matched a single 14KB concatenated string and
  `makeqstrdata.py` hit `assert False` ("qstr is too long"). Fixed by
  skipping `concat_strings()` entirely when `opt_E` is set. Normal
  compilation still concatenates string literals as required. New
  regression test: `test/test_pp_e_string_lines.c`. `test_micropython`
  now builds and its unix-port binary runs `print(1+2)` correctly.

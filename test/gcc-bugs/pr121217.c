...
/* ICE on invalid code involving bit fields with an undefined enum type;
 * fixed by r16-3029 (c: Fix ICE on invalid code involving bit fields).
 * gcc now reports the incomplete-type errors cleanly. */
typedef union{
        char *nordic_ref;
        unsigned long long int bit_number;
        enum PinMode mode : 2; /* { dg-error "field .mode. has incomplete type" } */
        unsigned char value;
    } s; typedef struct{
        union{
            char *nordic_ref;
...
} /* { dg-error "expected identifier or .(. at end of input" } */
//    13 |         } s;

...
/* ARM-specific -Warray-bounds warning test: dereferencing a fixed
 * low address (valid on microcontrollers with flash at address 0) must
 * not warn.  Compile-only: on x86 the deref would fault at runtime. */
int main() {
    return *((int *)4092);
}
// Compiling with

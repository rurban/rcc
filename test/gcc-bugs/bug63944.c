 ...
 */
/* { dg-do run { xfail *-*-* } } */
// NOTE: DR413 is still unimplemented (bug still NEW); with current gcc the
// partial designated-initializer override of .t.l/.t.a[1] incorrectly also
// clobbers .t.k back to 0, so l.t.k != 42 and the program below reaches
// abort() at runtime. That abort is the actual, still-reproducible bug.

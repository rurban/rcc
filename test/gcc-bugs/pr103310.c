...
// $ gcc -Wall -O2 -S -fdump-tree-original=/dev/stdout -fdump-tree-optimized=/dev/stdout pr103310.c
// pr103310.c: In function 'call_alias':
// pr103310.c:7:7: warning: the address of 'alias' will always evaluate as 'true' [-Waddress]
//     7 |   if (alias)
//       |       ^~~~~
//
...
// }
//
// At runtime, calling call_alias() with alias left undefined (weak,
// unresolved) crashes: the -O2 front-end folding above eliminates the
// null check on the weak symbol's address, so the call is unconditional.

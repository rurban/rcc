...
static int check_peep(const char *rcc, const char *src, int pid,
    ...
    int rc = system(cmd);
    if (rc != 0) {
        fprintf(stderr, "FAIL: [%s] O0 compile (rc=%d, errno=%d): %s\n", tag, rc, errno, cmd);
    ...
    rc = system(cmd);
    if (rc != 0) {
        fprintf(stderr, "FAIL: [%s] O1 compile (rc=%d, errno=%d): %s\n", tag, rc, errno, cmd);
    ...
    if (n0 < 0 || n1 < 0) {
        fprintf(stderr, "FAIL: [%s] cannot read asm\n", tag);
        if (!verbose) { remove(asm0); remove(asm1); }
        return -1;
    }
    int diff = n0 - n1;
    if (diff < 0) {
        fprintf(stderr, "FAIL: [%s] O1 larger (%d -> %d)\n", tag, n0, n1);
        if (verbose) {
    ...
    if (diff < min_diff) {
        fprintf(stderr, "ERROR: [%s] O0=%d O1=%d diff=%d (min=%d)\n",
...
#define ADD(_src, _tag, _min) do { srcs[n]=_src; tags[n]=_tag; mins[n]=_min; n++; } while(0)
    ...
    if (failed) fprintf(stderr, "%d peep pattern(s) failed\n", failed);
    return failed ? 1 : 0;
}

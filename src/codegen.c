...
void cg_def_label_sec(const char *name, int sec) {
    if (cg_dry_run) return;
...
int cg_dbg_line = 0;
int current_fn_stack_size = 0; // fn->stack_size of the function being generated
...
#define _BI_LOCAL(s) keyword_interned(s, sizeof(s) - 1)
char *bi_unreachable;
static char *kw_retbuf;

static void init_local_builtins(void) {
    static bool done = false;
    if (done) return;
    done = true;
    kw_retbuf = _BI_LOCAL("__retbuf");
    bi_unreachable = str_intern("__builtin_unreachable", 21);
}

// True if a block statement is a bare __builtin_unreachable() call.
...
static void emit_jmp_fixup(SecBuf *s, const char *label) {
...
static bool is_int128_like(Type *ty) {
    return ty && (ty->kind == TY_INT128 || ty->kind == TY_DECIMAL128 || (ty->kind == TY_BITINT && ty->size == 16));
}
...
// ---------------------------------------------------------------------------
// Decimal floating point (_Decimal32/64/128), IEEE 754-2008 BID encoding.
//
// Values are plain BID bit patterns: _Decimal32/64 fit a GP register,
// _Decimal128 is a 16-byte slot (like __int128). ALL arithmetic,
// comparisons and conversions go through the __bid_* runtime helpers from
// the bundled libdfp.a (lib/libdfp/rcc_dec_rt.c wrapper layer + the
// vendored libbid core); rcc's linker auto-links libdfp.a for decimal TUs
// (see main.c). The helpers use the kefir/gcc-compatible symbol names and
// a plain bit-pattern ABI: decimal32/64 in one GP reg, decimal128 as a
// two-GP-reg UINT128 struct.
// ---------------------------------------------------------------------------

static const char *dec_sfx(Type *ty) {
    return ty->kind == TY_DECIMAL32 ? "sd" : ty->kind == TY_DECIMAL64 ? "dd"
                                                                      : "td";
}

static const char *dec_op_name(int kind) {
    switch (kind) {
    case ND_ADD: return "__bid_add";
    case ND_SUB: return "__bid_sub";
    case ND_MUL: return "__bid_mul";
    default: return "__bid_div";
    }
}

static const char *dec_cmp_name(int kind) {
    switch (kind) {
    case ND_EQ: return "__bid_eq";
    case ND_NE: return "__bid_ne";
    case ND_LE: return "__bid_le";
    default: return "__bid_lt";
    }
}

// Move VReg `r` (a decimal32/64 BID value) into integer arg register N.
static void emit_dec_arg(VReg r, int n) {
#ifdef ARCH_ARM64
...
// Load low/high 64-bit words of the 16-byte UINT128 at slot `addr` into
// integer arg registers n and n+1 (the two-GP-reg UINT128 convention).
static void emit_dec128_arg(VReg addr, int n) {
#ifdef ARCH_ARM64
    static const Arm64Reg a64[] = {ARM64_X0, ARM64_X1, ARM64_X2, ARM64_X3, ARM64_X4};
    arm64_ldr_uoff(cg_sec, 3, ARM64_X16, REG(addr), 0); // ldr x16, [addr]
    arm64_ldr_uoff(cg_sec, 3, ARM64_X17, REG(addr), 1); // ldr x17, [addr, #8]
    asm_mov_phy_phy(cg_sec, a64[n], ARM64_X16, 1); // mov xN, x16
    asm_mov_phy_phy(cg_sec, a64[n + 1], ARM64_X17, 1); // mov xN+1, x17
#elif defined(_WIN32)
    // Windows x64: 16-byte struct by value is passed by hidden pointer;
    // our wrapper's UINT128 args use the SysV-style 2-reg convention only
    // on SysV; on Win64 the wrapper must receive them by pointer instead.
    static const X86Reg aw[] = {X86_RCX, X86_RDX, X86_R8, X86_R9};
    x86_mov_rr(cg_sec, 8, aw[n], cg_x86_reg[addr]); // movq %addr, %arg (pointer)
#else
    static const X86Reg al[] = {X86_RDI, X86_RSI, X86_RDX, X86_RCX, X86_R8};
    x86_mov_rm(cg_sec, 8, al[n], x86_mem(cg_x86_reg[addr], 0)); // movq (addr), %arg
    x86_mov_rm(cg_sec, 8, al[n + 1], x86_mem(cg_x86_reg[addr], 8)); // movq 8(addr), %arg+1
#endif
}

// Store the two-GP-reg UINT128 return (rax:rdx / x0:x1) into the 16-byte
// slot whose address is in VReg `dst`.
static void emit_dec128_ret(VReg dst) {
#ifdef ARCH_ARM64
    arm64_str_uoff(cg_sec, 3, ARM64_X0, REG(dst), 0); // str x0, [dst]
    arm64_str_uoff(cg_sec, 3, ARM64_X1, REG(dst), 1); // str x1, [dst, #8]
#elif defined(_WIN32)
...
static void emit_dec_call(char *fn) {
#ifdef ARCH_ARM64
    ...
    for (int i = 0; i < NUM_REGS; i++) {
        if (saved & (1 << i)) {
            ...
            int off = spill_offset(i);
            arm_pushed[i] = (spill_depth[i] > d0);
            arm_saved[i] = off;
            if (off <= 255)
                asm_stur_fp_phy(cg_sec, cg_arm_reg[i], off);
            else {
                VReg base = alloc_reg();
                asm_mov_imm(cg_sec, base, 8, off);
                asm_sub_reg_fp_reg(cg_sec, base, base, 8);
                asm_str_reg_off_phy(cg_sec, cg_arm_reg[i], base, 8, 0);
                free_reg(base);
            }
        }
    }
#else
    asm_push(cg_sec, X86_R10);
    asm_push(cg_sec, X86_R11);
#endif
    ...
    for (int i = 0; i < NUM_REGS; i++) {
        if (saved & (1 << i)) {
            int off = arm_saved[i];
            if (off <= 255)
                arm64_ldur(cg_sec, 1, cg_arm_reg[i], ARM64_X29, -off);
            else {
                VReg base = alloc_reg();
                asm_mov_imm(cg_sec, base, 8, off);
                asm_sub_reg_fp_reg(cg_sec, base, base, 8);
                VReg tmp = alloc_reg();
                asm_ldr_reg_off(cg_sec, tmp, base, 8, 0);
                asm_mov_phy_reg(cg_sec, cg_arm_reg[i], tmp, 1);
                free_reg(tmp);
                free_reg(base);
            }
            if (arm_pushed[i])
...
#else
    asm_pop(cg_sec, X86_R11);
    asm_pop(cg_sec, X86_R10);
#endif
}

// Generate a _Decimal32/64/128 node. Returns a GP VReg holding the BID
// bits (sd/dd) or the address of a 16-byte slot (td).
static VReg gen_decimal(Node *node) {
    if (!node) return R_NONE;
    if (opt_g) emit_loc(node);

    Type *ty = node->ty;
    // For comparisons the node's own type is int; for casts to non-decimal
    // (int/float) the decimal type comes from the source operand. The
    // decimal type always comes from the operands.
    if (!is_decimal(ty)) {
        if (node->lhs && node->lhs->ty && is_decimal(node->lhs->ty))
            ty = node->lhs->ty;
        else if (node->rhs && node->rhs->ty && is_decimal(node->rhs->ty))
            ty = node->rhs->ty;
    }
    bool is128 = ty->kind == TY_DECIMAL128;
    const char *sfx = dec_sfx(ty);

    switch (node->kind) {

    case ND_NUM: {
        // Compile-time-folded BID bits from the parser.
        if (is128) {
            VReg addr = alloc_wide_addr(16);
#ifdef ARCH_ARM64
            asm_mov_imm_phy(cg_sec, ARM64_X16, 8, (uint64_t)node->val);
            asm_str_reg_off_phy(cg_sec, ARM64_X16, addr, 8, 0);
            asm_mov_imm_phy(cg_sec, ARM64_X16, 8, (uint64_t)node->val2);
            asm_str_reg_off_phy(cg_sec, ARM64_X16, addr, 8, 8);
#else
            asm_movabs_phy(cg_sec, X86_RAX, (uint64_t)node->val);
            asm_mov_rax_mem(cg_sec, addr); // movq %rax, (addr)
            asm_movabs_phy(cg_sec, X86_RAX, (uint64_t)node->val2);
        ...
        VReg r = alloc_reg();
#ifdef ARCH_ARM64
        asm_mov_imm(cg_sec, r, 8, node->val);
#else
        asm_movabs_phy(cg_sec, cg_x86_reg[r], (uint64_t)node->val);
#endif
        return r;
    }
    ...
    case ND_LVAR:
    case ND_MEMBER: {
        // decimal32/64: load the BID value from the lvalue address into a
        // GP reg. decimal128: keep the address (slot-based, like int128).
        if (!is128) {
            VReg addr = gen_addr(node);
            VReg r = alloc_reg();
#ifdef ARCH_ARM64
            asm_ldr_reg_off(cg_sec, r, addr, ty->size, 0); // ldr w/x{r}, [addr]
#else
            asm_mov_mem_reg(cg_sec, r, addr, ty->size); // movl/movq (addr), %r
#endif
            free_reg(addr);
            return r;
        }
    ...
    case ND_CHAIN: {
        int r = gen(node->lhs);
        if (r >= 0) free_reg(r);
        return gen_decimal(node->rhs);
    }

    case ND_ASSIGN: {
        VReg rhs = gen_decimal(node->rhs);
        VReg lhs = gen_addr(node->lhs);
        if (is128) {
            emit_bitint_copy_bytes(lhs, rhs, 16);
            free_reg(rhs);
            return lhs;
        }
#ifdef ARCH_ARM64
        asm_str_reg_off(cg_sec, rhs, lhs, ty->size, 0); // str w/x{rhs}, [x{lhs}]
#else
        asm_mov_reg_mem(cg_sec, rhs, lhs, ty->size); // movl/movq %rhs, (lhs)
#endif
        free_reg(rhs);
        return lhs;
    }
    case ND_CAST: {
        ...
        Type *to = node->ty;
        if (is_decimal(from) && is_decimal(to)) {
            // decimal -> decimal: extend or truncate.
            ...
            const char *ts = dec_sfx(to);
            // width order: sd(4) < dd(8) < td(16)
            snprintf(fn, sizeof(fn), "__bid_%s%s%s2", from->size < to->size ? "extend" : "trunc",
                     fs, ts);
            if (from->size < to->size) {
                // extend sd->dd / sd->td / dd->td: source in one GP reg.
                if (to->kind == TY_DECIMAL128) {
                    // sd/dd -> td: result is 16 bytes (rax:rdx) -> slot.
                    VReg slot = alloc_wide_addr(16);
                    emit_dec_arg(src, 0);
                    emit_dec_call(fn);
                    emit_dec128_ret(slot);
                    free_reg(src);
                    return slot;
                }
                VReg dst = alloc_reg();
                ...
                emit_dec_call(fn);
#ifdef ARCH_ARM64
                asm_mov_phy_phy(cg_sec, cg_arm_reg[dst], ARM64_X0, 1);
#else
                x86_mov_rr(cg_sec, 8, cg_x86_reg[dst], X86_RAX);
#endif
                free_reg(src);
                return dst;
            } else {
                // trunc dd->sd / td->sd / td->dd: source 64-bit (dd) or
                // 128-bit slot (td). Route 128 through a slot.
                if (to->kind == TY_DECIMAL128) {
                    VReg slot = alloc_wide_addr(16);
        ...
        if (is_decimal(from) && is_integer(to) && to->kind != TY_BOOL) {
            ...
            char fn[64];
            const char *fs = dec_sfx(from);
            const char *ti = to->size == 8 ? "di" : "si";
            snprintf(fn, sizeof(fn), "__bid_fix%s%s%s", to->is_unsigned ? "uns" : "", fs, ti);
            VReg src = gen_decimal(node->lhs);
            if (from->kind == TY_DECIMAL128) {
                emit_dec128_arg(src, 0);
            } else {
        ...
        if (is_integer(from) && is_decimal(to)) {
            // int -> decimal: __bid_float{,uns}{si,di,ti}{sd,dd,td}
            char fn[64];
            const char *ts = dec_sfx(to);
            const char *si = from->size == 8 ? "di" : "si";
            snprintf(fn, sizeof(fn), "__bid_float%s%s%s", from->is_unsigned ? "uns" : "", si, ts);
        ...
        // decimal <-> binary float: the __bid_trunc* / __bid_extend*
        // helpers take/return the binary float in the FP register (XMM0
        // on x86 SysV), matching the platform float ABI — NOT a GP reg.
        if (is_decimal(from) && is_flonum(to)) {
            char fn[64];
            const char *fs = dec_sfx(from);
            const char *tf = to->size == 4 ? "sf" : "df";
            snprintf(fn, sizeof(fn), "__bid_trunc%s%s", fs, tf);
            VReg src = gen_decimal(node->lhs);
            if (from->kind == TY_DECIMAL128)
                emit_dec128_arg(src, 0);
            else
            ...
            VReg dst = alloc_reg();
#ifdef ARCH_ARM64
            asm_fmov_f2i(cg_sec, dst, 0, 1); // fmov x{dst}, d0
#else
            x86_movq_xmm_r(cg_sec, REG(dst), X86_XMM0); // movq %xmm0, %dst
#endif
            return dst;
        }
        if (is_flonum(from) && is_decimal(to)) {
            char fn[64];
            const char *fs = from->size == 4 ? "sf" : "df";
            const char *ts = dec_sfx(to);
            snprintf(fn, sizeof(fn), "__bid_extend%s%s", fs, ts);
            VReg src = gen(node->lhs);
#ifdef ARCH_ARM64
            asm_fmov_i2f(cg_sec, 0, src, 1); // fmov d0, x{src}
#else
            asm_movq_r_xmm(cg_sec, X86_XMM0, src); // movq %src, %xmm0
#endif
            free_reg(src);
            emit_dec_call(fn);
            if (to->kind == TY_DECIMAL128) {
        ...
        // decimal -> bool: nonzero BID bits are truthy (zero has all bits
        // clear; NaN/Inf have nonzero bits, hence truthy like C float).
        ...
        // Fallback: copy the raw bits (used for decimal32/64 identity
        // casts and casts to void-ish targets).
        {
            VReg src = gen_decimal(node->lhs);
            VReg dst = alloc_reg();
#ifdef ARCH_ARM64
    ...
    case ND_DIV: {
        char fn[64];
        snprintf(fn, sizeof(fn), "%s%s3", dec_op_name(node->kind), sfx);
        if (is128) {
            VReg a = gen_decimal(node->lhs); // 16-byte slot address
            VReg b = gen_decimal(node->rhs);
            VReg slot = alloc_wide_addr(16);
            emit_dec128_arg(a, 0);
            free_reg(a);
            emit_dec128_arg(b, 2);
    ...
    case ND_LE: {
        char fn[64];
        snprintf(fn, sizeof(fn), "%s%s2", dec_cmp_name(node->kind), sfx);
        if (is128) {
            VReg a = gen_decimal(node->lhs);
            VReg b = gen_decimal(node->rhs);
            emit_dec128_arg(a, 0);
    ...
    case ND_NEG: {
        // BID sign bit flip: bit 31 (sd) / 63 (dd) / 127 (td).
        if (is128) {
            VReg src = gen_decimal(node->lhs);
            VReg slot = alloc_wide_addr(16);
            ...
            VReg lo = alloc_reg();
#ifdef ARCH_ARM64
            asm_ldr_reg_off(cg_sec, hi, slot, 8, 8);
            asm_ldr_reg_off(cg_sec, lo, slot, 8, 0);
            asm_xor_imm(cg_sec, hi, 8, INT32_MIN); // flip bit 63 of high word
            asm_str_reg_off(cg_sec, hi, slot, 8, 8);
#else
            asm_mov_mem_rax(cg_sec, slot);
            asm_mov_mem8_rdx(cg_sec, slot);
            x86_xor_ri(cg_sec, 8, X86_RDX, INT32_MIN); // flip bit 63 of hi word
            asm_mov_rax_mem(cg_sec, slot);
            asm_mov_rdx_mem8(cg_sec, slot);
#endif
            free_reg(hi);
            free_reg(lo);
            return slot;
        ...
        VReg src = gen_decimal(node->lhs);
        uint64_t sign_bit = ty->size == 4 ? ((uint64_t)1 << 31) : ((uint64_t)1 << 63);
#ifdef ARCH_ARM64
        arm64_eor_imm(cg_sec, 1, REG(src), REG(src), sign_bit); // eor x{src}, x{src}, #sign
#else
        asm_movabs_phy(cg_sec, X86_RAX, sign_bit);
        x86_xor_rr(cg_sec, 8, cg_x86_reg[src], X86_RAX); // xor %rax, %src
#endif
        return src;
    }

    default:
        // Unhandled decimal op: fall back to the generic path (likely a
        // diagnostic will surface; most scalar ops route here first).
...
static VReg gen_funcall(Node *node, VReg hidden_ret_reg) {
    ...
    */
    ...
    if (call_target && !has_hidden_retbuf) {
        VReg bi_result = gen_builtin_call(node, call_target, gen);
        if (bi_result != R_NONE)
            return bi_result;
    }
...
#endif
        // Win64 has no register-argument staging (unlike the SysV loop
        // below): each arg was computed directly into a register that a
        // sibling arg's placement reload -- or that arg's own evaluation
        // after the first was spilled+freED -- can legitimately reclaim,
        // silently aliasing two argument registers onto one physical index.
        // Placing the first arg then clobbers the second. Observed for real
        // under mingw (csmith 1105826-fail_refmismatch_156): integer-literal
        // arguments (the shift amounts 6 and 13 in
        //   (*p) = safe_add(safe_mul(safe_rshift(+(a[4]--), 6),
        //                            safe_lshift((*q) ^= r, 13)), -5))
        // had their register reused by the sibling operand's spill reload,
        // so the literals arrived as stale garbage. Always archive each
        // register-passed arg into its own stack slot here, then reload it
        // into a fresh register at placement time (below). Win64 passes at
        // most 4 GP args, so this costs at most 4 slots.
        {
            fn_struct_ret_off += 8;
            if (fn_struct_ret_off > fn_struct_ret_total)
                fn_struct_ret_total = fn_struct_ret_off;
    ...
    for (int i = 0; i < reg_nargs; i++) {
        ...
        if (arg_regs[i] < 0) {
            // Staged argument: reload its archived value into a fresh
            // register (unique per arg, so no two args alias one register).
            arg_regs[i] = alloc_reg();
        ...
        // Release the (possibly staging-reloaded) register. When it was a
        // recycled outer VReg (spilled_regs still set from an earlier
        // spill), don't restore -- the outer expression needs its value in
        // the spill slot so the binary-op same-register combining can
        // recover it after the call. Mirrors the SysV loop's release.
...
VReg gen_addr(Node *node) {
    ...
    case ND_CAST:
        if (node->ty && node->ty->is_vector) {
            // Vector cast: same-size vector -> vector is a bitcast (the
            // address is unchanged); scalar -> vector broadcasts the scalar
            // to every lane (GCC semantics), materialized in a slot.
            if (node->lhs && node->lhs->ty && node->lhs->ty->is_vector)
                return gen_addr(node->lhs);
#ifdef ARCH_ARM64
            // NEON DUP broadcast. Float scalars are double bit patterns in
            // GP registers — narrow to single first (arm64 dup takes the
            // low bytes of the GP register).
            Type *elem = node->ty ? node->ty->base : NULL;
            int esz = elem ? (int)elem->size : 4;
            VReg r = gen(node->lhs);
            VReg dst = alloc_int128_addr();
            if (elem && is_flonum(elem) && esz == 4) {
                asm_fmov_i2f(cg_sec, 0, r, 1); // fmov d0, x{r}
                asm_fcvt(cg_sec, 0, 1, ARM64_S0, ARM64_D0); // fcvt s0, d0
                asm_fmov_f2i(cg_sec, r, ARM64_S0, 0); // fmov w{r}, s0
            }
            // DUP (general): broadcast the low esz bytes of w{r} to every
            // lane. arm64_dup_gen only supports 32/64-bit; byte and
            // halfword elements need imm5 = esz directly.
            if (esz == 8 || esz == 4)
                arm64_dup_gen(cg_sec, esz == 8 ? 3 : 2, ASM_Q0, REG(r));
            else
                secbuf_emit32le(cg_sec, 0x4E000C00u | ((uint32_t)esz << 16) | ((uint32_t)(REG(r) & 31) << 5) | (uint32_t)(ASM_Q0 & 31));
...
// Store a forward branch fixup. Uses fwd_off (direct patch) if non-NULL,
// otherwise shared_label (hash table) if non-NULL,
// otherwise auto-generates and resolves immediately.
static void cg_add_fwd(size_t instr_off, int type, size_t *fwd_off, const char *shared_label) {
    if (fwd_off) {
        *fwd_off = instr_off;
    } else if (shared_label) {
        asm_fixup_add(cg_sec, instr_off, shared_label, type);
    } else {
        int c = ++rcc_label_count;
        const char *lbl = format(".L.gcbi.%d", c);
        asm_fixup_add(cg_sec, instr_off, lbl, type);
        cg_def_label(lbl);
    }
}
...
//   fwd_off != NULL → store single branch offset for direct patching via asm_patch_branch.
//   fwd_off == NULL → auto-generate label and resolve via cg_def_label (hash table fallback).
static void gen_cond_branch_inv(Node *cond, size_t *fwd_off, const char *shared_label) {
    if (cond->kind == ND_LOGAND) {
        // Two recursive calls share the same label.
        // Only call cg_def_label when we own the label;
        // if the caller provided shared_label, defer to the caller.
        bool own_label = (shared_label == NULL);
        if (own_label) {
            int c = ++rcc_label_count;
            shared_label = format(".L.gcbi.%d", c);
        }
        gen_cond_branch_inv(cond->lhs, NULL, shared_label);
        gen_cond_branch_inv(cond->rhs, NULL, shared_label);
        if (own_label)
            cg_def_label(shared_label);
        return;
    }
    if (cond->kind == ND_LOGOR) {
        ...
        free_reg(lhs);
#ifdef ARCH_ARM64
        size_t o = asm_jcc_label(cg_sec, ARM64_NE); // b.ne skip
#else
        size_t o = asm_jcc_label(cg_sec, X86_NE); // jne skip
#endif
        // If lhs was false, check rhs; if rhs is also false, branch to target
        gen_cond_branch_inv(cond->rhs, fwd_off, shared_label);
        asm_patch_branch(cg_sec, o, cg_sec->len, 1); // patch skip to here
        return;
    }
    if (cond->kind == ND_EQ || cond->kind == ND_NE || cond->kind == ND_LT || cond->kind == ND_LE) {
        ...
        if ((cond->lhs->ty && is_decimal(cond->lhs->ty)) ||
            (cond->rhs && cond->rhs->ty && is_decimal(cond->rhs->ty))) {
            // decimal comparison: gen_decimal returns a boolean VReg (0 or 1)
            VReg r = gen_decimal(cond);
            asm_cmp_zero(cg_sec, r, 4); // cmpl $0, rr
            free_reg(r);
#ifdef ARCH_ARM64
            size_t o = asm_jcc_label(cg_sec, ARM64_EQ); // b.eq target
        ...
        if (is_flonum(cond->lhs->ty)) {
            ...
            if (cond->kind == ND_EQ) {
                ...
                if (!lbl) {
                    int c = ++rcc_label_count;
                    lbl = format(".L.gcbi.%d", c);
                    own_lbl = true;
                }
                {
                    size_t o = asm_jcc_label(cg_sec, ARM64_NE); // b.ne label
                    asm_fixup_add(cg_sec, o, lbl, 1);
                }
                {
                ...
                size_t o = asm_jcc_label(cg_sec, ARM64_PL); // b.pl target
                cg_add_fwd(o, 1, fwd_off, shared_label);
            } else {
                size_t o = asm_jcc_label(cg_sec, ARM64_HI); // b.hi target
                cg_add_fwd(o, 1, fwd_off, shared_label);
            }
#else
...
#ifdef ARCH_ARM64
        if (cond->kind == ND_EQ) {
            size_t o = asm_jcc_label(cg_sec, ARM64_NE); // b.ne target
            cg_add_fwd(o, 1, fwd_off, shared_label);
        } else if (cond->kind == ND_NE) {
            size_t o = asm_jcc_label(cg_sec, ARM64_EQ); // b.eq target
            cg_add_fwd(o, 1, fwd_off, shared_label);
        } else if (cond->kind == ND_LT) {
            size_t o = asm_jcc_label(cg_sec, use_unsigned_cmp(cond) ? ARM64_HS : ARM64_GE); // b.hs/b.ge target
            cg_add_fwd(o, 1, fwd_off, shared_label);
        } else {
            size_t o = asm_jcc_label(cg_sec, use_unsigned_cmp(cond) ? ARM64_HI : ARM64_GT); // b.hi/b.gt target
            cg_add_fwd(o, 1, fwd_off, shared_label);
        }
#else
        if (cond->kind == ND_EQ) {
            size_t o = asm_jcc_label(cg_sec, X86_NE); // jne target
            cg_add_fwd(o, 1, fwd_off, shared_label);
        } else if (cond->kind == ND_NE) {
            size_t o = asm_jcc_label(cg_sec, X86_E); // je target
            cg_add_fwd(o, 1, fwd_off, shared_label);
        } else if (cond->kind == ND_LT) {
            size_t o = asm_jcc_label(cg_sec, use_unsigned_cmp(cond) ? X86_AE : X86_GE); // jae/jge target
            cg_add_fwd(o, 1, fwd_off, shared_label);
        } else {
            size_t o = asm_jcc_label(cg_sec, use_unsigned_cmp(cond) ? X86_A : X86_G); // ja/jg target
            cg_add_fwd(o, 1, fwd_off, shared_label);
        }
#endif
        return;
    ...
    cg_add_fwd(cond_off, 1, fwd_off, shared_label);
#else
    asm_cmp_zero(cg_sec, r, sz);
    free_reg(r);
...
VReg gen_scalar_addr(Node *node) {
...
VReg gen(Node *node) {
    ...
    // Dispatch _Decimal32/64/128 nodes to gen_decimal() (BID runtime calls)
    if (node->ty && is_decimal(node->ty))
        return gen_decimal(node);
    // Decimal comparisons: the ND_EQ/NE/LT/LE node's own type is int
    // (comparison result), so dispatch on the operand types.
...
#undef BF_STORE
    ...
    case ND_CAST: {
        ...
        // Decimal casts: _Decimal* <-> integer / binary float / _Decimal*.
        // The result type may be non-decimal (int/float), so dispatch on
        // operand types here before the generic scalar path.
        if ((node->lhs && node->lhs->ty && is_decimal(node->lhs->ty)) ||
            (node->ty && is_decimal(node->ty)))
            return gen_decimal(node);
        VReg r = gen(node->lhs);
...
*/
    ...
    case ND_IF: {
        ...
        size_t else_jmp = (size_t)-1;
        size_t end_jmp = (size_t)-1;
        if (node->els) {
            const char *else_label = format(".L.else.%d", c);
            gen_cond_branch_inv(node->cond, &else_jmp, else_label);
            VReg r1 = gen(node->then);
            if (r1 != -1) free_reg(r1);
            end_jmp = asm_jmp_label(cg_sec); // jmp end
            asm_patch_branch(cg_sec, else_jmp, cg_sec->len, 1); // else:
            cg_def_label(else_label);
            VReg r2 = gen(node->els);
            if (r2 != -1) free_reg(r2);
            asm_patch_branch(cg_sec, end_jmp, cg_sec->len, 0); // end:
            cg_def_label(end_label);
        } else {
            gen_cond_branch_inv(node->cond, &end_jmp, end_label);
            VReg r1 = gen(node->then);
            if (r1 != -1) free_reg(r1);
            asm_patch_branch(cg_sec, end_jmp, cg_sec->len, 1); // end:
            cg_def_label(end_label);
        }
        return -1;
    }
    case ND_FOR: {
        ...
        cg_def_label(cont_label); // continue label
        if (node->inc) {
            VReg r_inc = gen(node->inc);
            if (r_inc != -1) free_reg(r_inc);
        }
        asm_b_back(cg_sec, begin_pos); // b begin
        asm_patch_branch(cg_sec, end_jmp, cg_sec->len, 1); // patch cond branch
        cg_def_label(end_label); // end label (for break)
        ctrl_depth--;
        return -1;
    }
    case ND_DO: {
    ...
    for (TLItem *item = prog->items; item; item = item->next) {
        ...
        for (LVar *var = fn->params; var; var = var->param_next) {
            ...
            if (is_flonum(var->ty)) {
                ...
                } else if (var->ty->kind == TY_INT128 || var->ty->kind == TY_DECIMAL128) {
                    // __int128/_Decimal128 passed by pointer on Windows x64.
                ...
                if (is_flonum(var->ty)) {
                    ...
                    // __int128/_Decimal128 on stack: slot holds a pointer.
        ...
        int n_ld_deferred = 0;
        {
            for (LVar *var = fn->params; var; var = var->param_next) {
                ...
                if (hfa_count > 0 && fp_param + hfa_count <= 8) {
                    ...
                    // int128 / _Decimal128: two consecutive GP registers
                    if (gp_param + 1 < 8) {
                        /* golden:
        ...
        // Save incoming params from ABI regs to stack slots
        {
            ...
            for (LVar *var = fn->params; var; var = var->param_next) {
                ...
                // Linux SysV: __int128 / _Decimal128 occupy two consecutive
                // GP registers/stack slots
                if (var->ty->kind == TY_INT128 || var->ty->kind == TY_DECIMAL128) {
                    if (gp + 1 < max_gp) {

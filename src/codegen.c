...
static void emit_alloca(void) {
    ...
    asm_push(cg_sec, X86_RDX); // pushq %rdx  (restore return addr)
    asm_ret(cg_sec); // ret
#endif
}
...
static VReg gen_funcall(Node *node, VReg hidden_ret_reg) {
        ...
        if (is_add_overflow || is_sub_overflow || is_mul_overflow || is_mul_overflow_p) {
            ...
            if (is_add_overflow) {
                ...
                if (res_sz == sz) {
                    ...
                    if (res_unsigned) {
                        if (res_sz == 2) {
                            ...
                            asm_shr_imm(cg_sec, r_result, 8, (uint8_t)(reg64[ra]));
                            asm_cmp_zero(cg_sec, r_result, 8);
                            asm_cset(cg_sec, r_result, ARM64_NE);
                        }
                    } else {
                        int rt = alloc_reg();
                        asm_sxtb_phy(cg_sec, rt, rt); // sxt%c %s, %s
                        asm_cmp_reg_reg(cg_sec, rt, ra, 8);
                        asm_cset(cg_sec, r_result, ARM64_NE);
                        free_reg(rt);
                    }
                } else {
                    ...
                    if (sz == 4) {
                        if (arga->ty && arga->ty->is_unsigned)
                            asm_uxtw_phy(cg_sec, REG(rt), REG(rt)); // uxtw %s, %s
                        else
                    ...
                    if (res_unsigned && (!arga->ty || !arga->ty->is_unsigned || !argb->ty || !argb->ty->is_unsigned)) {
                        ...
                        asm_cset(cg_sec, r_result, ARM64_MI);
                    } else {
                        (void)0 /* FIXME: cset with var cond */;
                    }
                }
                ...
                if (res_sz >= sz * 2) {
                    ...
                    asm_movq_zero(cg_sec, r_result); // mov %s, #0
                } else if (res_sz == sz) {
                    ...
                    if (sz == 8) {
                        ...
                        if (res_unsigned) {
                            ...
                            (void)0 /* FIXME: NEON */;
                            asm_movsx(cg_sec, r2, ra, 8, 4);
                            asm_cmp_reg_reg(cg_sec, r2, ra, 8);
                            asm_cset(cg_sec, r_result, ARM64_NE);
                        }
                    }
                    free_reg(r2);
                    ...
                    if (res_unsigned) {
                        if (sz == 8) {
                            asm_cmp_zero(cg_sec, r2, 8);
                            asm_cset(cg_sec, r_result, ARM64_NE);
                            asm_shr_imm(cg_sec, r2, 8, (uint8_t)(reg64[ra]));
                            asm_cmp_zero(cg_sec, r2, 8);
                            asm_cinc_ne(cg_sec, r3, r3); // cinc %s, %s, ne
                        } else {
                            asm_shr_imm(cg_sec, r2, 8, (uint8_t)(reg64[ra]));
                            asm_cmp_zero(cg_sec, r2, 8);
                            asm_cset(cg_sec, r_result, ARM64_NE);
                        ...
                        int r3 = alloc_reg();
                        if (sz == 8) {
                            asm_sar_imm(cg_sec, r3, 8, (uint8_t)(reg64[ra]));
                            asm_cmp_reg_reg(cg_sec, r2, r3, 8);
                            asm_cset(cg_sec, r_result, ARM64_NE);
                            asm_sbfx_0(cg_sec, r3, ra, res_sz * 8); // sbfx r3, ra, #0, #res_sz*8
                            asm_cmp_reg_reg(cg_sec, r3, ra, 8);
                            asm_cinc_ne(cg_sec, r3, r3); // cinc %s, %s, ne
                        } else {
                            asm_sar_imm(cg_sec, r2, 8, (uint8_t)(reg64[ra]));
                            asm_shr_imm(cg_sec, r3, 8, (uint8_t)(reg64[ra]));
                            asm_cmp_reg_reg(cg_sec, r2, r3, 8);
                            asm_cset(cg_sec, r_result, ARM64_NE);
                        }
                        free_reg(r3);
    ...
    if (!call_target) {
        ...
        if (n_save > 0) {
            ...
            asm_sub_imm(cg_sec, STACK_REG, 8, indirect_save_size); // sub sp, sp, #indirect_save_size
            int slot = 0;
            if (indirect_save_x8)
                (void)0 /* FIXME: ldr/str phy/off */;
            for (int j = 0; j < 8; j++)
    ...
    for (int i = nargs - 1; i >= 0; i--) {
        ...
        if (argv[i]->ty->kind == TY_LDOUBLE) {
            VReg r = gen(argv[i]);
            x86_mov_mr(cg_sec, 8, x86_mem(X86_RSP, arg_stack_idx[i] * 8), REG(r)); // movq %s, %d(%rsp)
    ...
    case ND_CAST:
        if (node->ty && (node->ty->kind == TY_STRUCT || node->ty->kind == TY_UNION || node->ty->kind == TY_COMPLEX || node->ty->kind == TY_ARRAY)) {
            ...
            if (node->ty->kind == TY_COMPLEX && node->lhs->ty && node->lhs->ty->kind == TY_COMPLEX &&
                ...
                VReg dst = alloc_reg();
#ifdef ARCH_ARM64
                asm_ldr_x_fp_neg_off(cg_sec, dst, (int32_t)result_off ^ (int32_t)-result_off ? (int32_t)-result_off : (int32_t)-result_off);
                asm_ldr_x_fp_neg_off(cg_sec, dst, result_off); // FIXME: need proper sub fp, #off
                asm_stur_phy(cg_sec, REG(dst), ARM64_X29, 3, -result_off); // FIXME: this is wrong
#else
                asm_lea_rbp_reg(cg_sec, dst, result_off); // lea -result_off(%rbp), dst
#endif
...
static int alloc_int128_addr(void) {
    ...
    if (slot <= 4095) {
        ...
        asm_sub_reg_fp_reg(cg_sec, ta, ta, 8); // sub ta, x29, ta
    }
#else
...
#else
    asm_mov_reg_reg(cg_sec, 0, val, 8);
    asm_mov_rax_mem(cg_sec, addr); // movq val, (addr)
    if (is_unsigned) {
        asm_movq_zero(cg_sec, addr);
    } else {
        asm_mov_reg_reg(cg_sec, 0, val, 8);
...
static void gen_int128_nonzero(int addr, int result) {
    ...
    (void)0 /* FIXME: ldr/str phy/off */;
    asm_orr_x16(cg_sec, t, t); // orr %s, %s, x16
    asm_cmp_zero(cg_sec, t, 8);
    asm_cset(cg_sec, result, ARM64_NE);
    free_reg(t);
#else
    asm_mov_reg_reg(cg_sec, 0, addr, 8);
    (void)0 /* FIXME: sized alu op */;
    asm_setcc(cg_sec, 0, X86_NE);
    asm_movzx(cg_sec, result, 0, 4, 1);
#endif
}
...
static int gen_int128(Node *node) {
...
#else
        if (lo == (int32_t)lo)
            (void)0 /* FIXME: movq imm to (addr) */; // movq $%lld, (%s)
        else {
    ...
    case ND_ASSIGN: {
        ...
        int lhs_a = gen_addr(node->lhs);
        int rhs_a = gen_to_int128(node->rhs);
        asm_mov_reg_reg(cg_sec, 0, rhs_a, 8);
        asm_mov_mem8_rdx(cg_sec, rhs_a); // movq 8(%s), %%rdx
        asm_mov_reg_reg(cg_sec, lhs_a, 0, 8);
        asm_mov_rdx_mem8(cg_sec, lhs_a); // movq %%rdx, 8(%s)
#endif
        free_reg(rhs_a);
        return lhs_a;
    }
    ...
    case ND_CAST: {
        ...
        if (from && is_flonum(from)) {
            ...
            if (to->is_unsigned) {
                ...
                (void)0 /* FIXME: float op */;
                asm_jb_label_to(cg_sec, format(".L.f2i128u.%d", c)); // jb .L.f2i128u.%d
                (void)0 /* FIXME: float op */;
                x86_cvttsd2si(cg_sec, 8, X86_RAX, X86_XMM0); // cvttsd2si %%xmm0, %%rax
                (void)0 /* FIXME: movq special imm */;
                (void)0 /* FIXME: sized alu op */;
                asm_jmp_label_to(cg_sec, format(".L.f2i128u_lo.%d", c)); // jmp .L.f2i128u_lo.%d
    ...
    case ND_NEG: {
        ...
        asm_mov_mem8_rdx(cg_sec, src); // movq 8(%s), %%rdx
        asm_mov_reg_reg(cg_sec, dst, 0, 8);
        asm_mov_rdx_mem8(cg_sec, dst); // movq %%rdx, 8(%s)
    ...
    case ND_ADD: {
        ...
        asm_mov_mem8_rdx(cg_sec, rhs); // movq 8(%s), %%rdx
        (void)0 /* FIXME: sized alu op */;
        asm_adcq_mem8_rdx(cg_sec, rhs); // adcq 8(%s), %%rdx
        asm_mov_reg_reg(cg_sec, dst, 0, 8);
        asm_mov_rdx_mem8(cg_sec, dst); // movq %%rdx, 8(%s)
#endif
        free_reg(lhs);
    ...
    case ND_SUB: {
        ...
        asm_mov_mem8_rdx(cg_sec, rhs); // movq 8(%s), %%rdx
        (void)0 /* FIXME: sized alu op */;
        asm_sbbq_mem8_rdx(cg_sec, rhs); // sbbq 8(%s), %%rdx
        asm_mov_reg_reg(cg_sec, dst, 0, 8);
        asm_mov_rdx_mem8(cg_sec, dst); // movq %%rdx, 8(%s)
#endif
        free_reg(lhs);
    ...
    case ND_MUL: {
        ...
        int a_lo = alloc_reg(), a_hi = alloc_reg(), b_lo = alloc_reg(), b_hi = alloc_reg();
        (void)0 /* FIXME: indirect mov */;
        asm_mov_mem8_reg(cg_sec, a_hi, lhs); // movq 8(lhs), a_hi
        (void)0 /* FIXME: indirect mov */;
        asm_mov_mem8_reg(cg_sec, b_hi, rhs); // movq 8(rhs), b_hi
        free_reg(lhs);
        free_reg(rhs);
        int dst = alloc_int128_addr();
        // rdx:rax = a_lo * b_lo (unsigned)
        asm_mov_reg_reg(cg_sec, 0, a_lo, 8);
        asm_mulq(cg_sec, a_hi); // mulq %s
        asm_mov_reg_reg(cg_sec, dst, 0, 8);
        asm_mov_rdx_mem8(cg_sec, dst); // movq %%rdx, 8(%s)
        // result_hi += a_hi * b_lo
        asm_imulq_reg_reg(cg_sec, b_lo, a_hi); // imulq %s, %s
        (void)0 /* FIXME: sized alu op */;
        // result_hi += a_lo * b_hi
        asm_imulq_reg_reg(cg_sec, b_lo, a_hi); // imulq %s, %s
        (void)0 /* FIXME: sized alu op */;
    ...
    case ND_MOD: {
        ...
        (void)0 /* FIXME: ldr/str phy/off */;
        (void)0 /* FIXME: ldr/str phy/off */;
        asm_sub_imm(cg_sec, STACK_REG, 8, 32); // sub sp, sp, #32
        (void)0 /* FIXME: ldr/str phy/off */;
        asm_bl(cg_sec, asm_sym_name(func_label(name))); // bl %s
        (void)0 /* FIXME: ldr/str phy/off */;
        (void)0 /* FIXME: ldr/str phy/off */;
        (void)0 /* FIXME: ldr/str phy/off */;
        (void)0 /* FIXME: ldr/str phy/off */;
        asm_add_imm(cg_sec, STACK_REG, 8, 32); // add sp, sp, #32
#else
        asm_mov_reg_reg(cg_sec, 7, lhs, 8);
        asm_mov_reg_reg(cg_sec, 6, lhs, 8);
        (void)0 /* FIXME: indirect mov */;
        asm_mov_reg_reg(cg_sec, 1, rhs, 8);
        emit_direct_call(fn);
        asm_mov_reg_reg(cg_sec, dst, 0, 8);
        asm_mov_rdx_mem8(cg_sec, dst); // movq %%rdx, 8(%s)
#endif
    ...
    case ND_SHR: {
        ...
        if (is_rhs_const) {
            ...
            if (shift == 0) {
                ...
                if (shift < 64) {
                    ...
                    asm_shl_imm(cg_sec, t1, 8, (uint8_t)(reg64[t1]));
                } else if (shift == 64) {
                    asm_mov_reg_reg(cg_sec, t2, t1, 8);
                    asm_mov_xzr_phy(cg_sec, REG(t1)); // mov %s, xzr
                } else {
                    asm_shl_imm(cg_sec, t2, 8, (uint8_t)(reg64[t1]));
                    asm_mov_xzr_phy(cg_sec, REG(t1)); // mov %s, xzr
                }
            } else {
                if (shift < 64) {
        ...
        if (node->rhs->kind == ND_NUM) {
            ...
            if (is_shl) {
                ...
                (void)0 /* FIXME: testb */;
                asm_jz_label_to(cg_sec, format(".L.shl128.%d", c)); // jz .L.shl128.%d
                asm_mov_rax_rdx(cg_sec); // movq %%rax, %%rdx
                asm_movl_zero(cg_sec, 0);
                cg_def_label(format(".L.shl128.%d", ++rcc_label_count)); // .L.xxx.rcc_label_count++
            } else {
                asm_shrdq_cl(cg_sec); // shrdq %%cl, %%rdx, %%rax
                asm_shift_rdx_cl(cg_sec, is_unsigned); // %s %%cl, %%rdx
                (void)0 /* FIXME: testb */;
                asm_jz_label_to(cg_sec, format(".L.shr128.%d", c)); // jz .L.shr128.%d
                asm_mov_rdx_rax(cg_sec); // movq %%rdx, %%rax
                if (is_unsigned)
                    asm_movl_zero(cg_sec, 2);
                else
    ...
    case ND_BITXOR: {
        ...
        asm_ldr_reg_off(cg_sec, t3, rhs, 8, 0);
        (void)0 /* FIXME: ldr/str phy/off */;
        (void)0 /* FIXME: var branch/tst */; // %s %s, %s, %s
        (void)0 /* FIXME: var branch/tst */; // %s %s, %s, %s
        asm_str_reg_off(cg_sec, t1, dst, 8, 0);
        (void)0 /* FIXME: ldr/str phy/off */;
        free_reg(t1);
        ...
        asm_mov_reg_reg(cg_sec, 0, lhs, 8);
        asm_mov_mem8_rdx(cg_sec, rhs); // movq 8(%s), %%rdx
        asm_op_mem_rax(cg_sec, op, rhs); // op (rhs), %%rax
        asm_op_mem8_rdx(cg_sec, op, rhs); // op 8(rhs), %%rdx
        asm_mov_reg_reg(cg_sec, dst, 0, 8);
        asm_mov_rdx_mem8(cg_sec, dst); // movq %%rdx, 8(%s)
#endif
        free_reg(lhs);
    ...
    case ND_LE: {
        ...
        if (node->kind == ND_EQ || node->kind == ND_NE) {
            ...
            (void)0 /* FIXME: branch/call: b.%s .L.cmp128y.%d */;
            (void)0 /* FIXME: branch/call: b.%s .L.cmp128n.%d */;
            asm_cmp_phy_phy(cg_sec, t1, t1); // cmp %s, %s
            (void)0 /* FIXME: branch/call: b.%s .L.cmp128y.%d */;
#ifdef ARCH_ARM64
            (void)cg_sec; /* FIXME: ARM64 branch — use arm64_b/bcond with proper context */
#endif
            cg_def_label(format(".L.f2i128u_lo.%d", c)); // .L.xxx.c
        ...
        if (node->kind == ND_EQ || node->kind == ND_NE) {
            ...
            asm_mov_mem8_rdx(cg_sec, rhs); // movq 8(%s), %%rdx
            (void)0 /* FIXME: cmp variant */;
            (void)0 /* FIXME: cmp128y branch */; // %s .L.cmp128y.%d
            (void)0 /* FIXME: cmp128n branch */; // %s .L.cmp128n.%d
            asm_mov_reg_reg(cg_sec, 0, lhs, 8);
            (void)0 /* FIXME: cmp variant */;
            (void)0 /* FIXME: cmp128y branch */; // %s .L.cmp128y.%d
            cg_def_label(format(".L.f2i128u_lo.%d", c)); // .L.xxx.c
            asm_xorl_reg_reg(cg_sec, result, result); // xorl %s, %s
            asm_jmp_label_to(cg_sec, format(".L.cmp128e.%d", c)); // jmp .L.cmp128e.%d
            cg_def_label(format(".L.f2i128u_lo.%d", c)); // .L.xxx.c
...
#else
        asm_cmp_zero(cg_sec, cond_r, node->cond->ty ? node->cond->ty->size : 4);
        asm_je_label_to(cg_sec, format(".L.else.%d", c)); // je .L.else.%d
#endif
        free_reg(cond_r);
...
#endif
#else
        asm_mov_reg_reg(cg_sec, 0, then_a, 8);
        asm_mov_mem8_rdx(cg_sec, then_a); // movq 8(%s), %%rdx
        asm_mov_reg_reg(cg_sec, dst, 0, 8);
        asm_mov_rdx_mem8(cg_sec, dst); // movq %%rdx, 8(%s)
        asm_jmp_label_to(cg_sec, format(".L.end.%d", c)); // jmp .L.end.%d
#endif
        free_reg(then_a);
        cg_def_label(format(".L.f2i128u_lo.%d", c)); // .L.xxx.c
...
#else
        asm_mov_reg_reg(cg_sec, 0, else_a, 8);
        asm_mov_mem8_rdx(cg_sec, else_a); // movq 8(%s), %%rdx
        asm_mov_reg_reg(cg_sec, dst, 0, 8);
        asm_mov_rdx_mem8(cg_sec, dst); // movq %%rdx, 8(%s)
#endif
        free_reg(else_a);
    ...
    case ND_VA_ARG: {
        ...
        (void)0 /* FIXME: cmp variant */;
        asm_ja_label_to(cg_sec, format(".L.va128_stk.%d", c)); // ja .L.va128_stk.%d
        // Register save area: load both 64-bit halves
        (void)0 /* FIXME: indirect mov */;
        (void)0 /* FIXME: sized alu op */;
        (void)0 /* FIXME: sized alu op */;
        (void)0 /* FIXME: indirect mov */;
        asm_mov_reg_reg(cg_sec, addr, 0, 8);
        asm_mov_mem8_rcx_rax(cg_sec); // movq 8(%%rcx), %%rax
        asm_mov_reg_reg(cg_sec, addr, 0, 8);
        asm_jmp_label_to(cg_sec, format(".L.va128_d.%d", c)); // jmp .L.va128_d.%d
        ...
        (void)0 /* FIXME: sized alu op */;
        (void)0 /* FIXME: lea */;
        asm_mov_rdx_mem8(cg_sec, addr); // movq %%rdx, 8(%s)
        (void)0 /* FIXME: indirect mov */;
        asm_mov_reg_reg(cg_sec, addr, 0, 8);
        asm_mov_mem8_rcx_rax(cg_sec); // movq 8(%%rcx), %%rax
        asm_mov_reg_reg(cg_sec, addr, 0, 8);
...
*/
    ...
    if ((node->kind == ND_ADD || node->kind == ND_SUB || node->kind == ND_MUL || node->kind == ND_DIV) && is_complex(node->ty)) {
        ...
        if (is_flonum(node->ty->base)) {
#ifdef ARCH_ARM64
            const char *ldr_sfx = base_sz == 4 ? "s" : "d";
            const char *fop = "";
            if (node->kind == ND_ADD) fop = "fadd";
        ...
        int stack_param = 0;
        {
            ...
            for (LVar *var = fn->params; var; var = var->param_next) {
                ...
                if (hfa_count > 0 && fp_param + hfa_count <= 8) {
                    ...
                    (void)0 /* FIXME: branch/call: b.eq .L.pcopy_end.%d */;
                    asm_dec(cg_sec, 9, 8);
                    (void)0 /* FIXME: sized ld/st */;
                    (void)0 /* FIXME: sized ld/st */;
                    (void)0 /* FIXME: branch/call: b .L.pcopy.%d */;
                    cg_def_label(format(".L.f2i128u_lo.%d", c)); // .L.xxx.c
                    stack_param += (sz + 7) / 8;
    ...
    if (has_dtor) {
        ...
        for (TLItem *item = prog->items; item; item = item->next) {
            if (item->kind == TL_FUNC && item->fn->is_destructor) {
                ...
                asm_bl(cg_sec, "___cxa_atexit"); // bl ___cxa_atexit
